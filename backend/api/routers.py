    ...
    async def update_task(id: str, request: Request, task: TaskUpdateModel = Body(...)):
        ...
        if len(task) >= 1:
            ...
            if update_result.modified_count == 1:
                if (
                    updated_task := await request.app.mongodb["tasks"].find_one(
                        {"_id": id}
                    )
                ) is not None:
                    return JSONResponse(
                        status_code=status.HTTP_201_CREATED, content=updated_task
                    )
        ...
        if (
            ...
            return JSONResponse(
                status_code=status.HTTP_201_CREATED, content=existing_task
            )
